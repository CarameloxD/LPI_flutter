...
class _deleteClassState extends State<deleteClass> {
  ...
  Future<int> attemptDelete(String id, BuildContext context) async {
    ...
    final response = await http.delete(
      Uri.parse('http://10.0.2.2:8081/api/v1/class/$id'),
    );
        ...
        body: ListView.builder(
          ...
          itemBuilder: (BuildContext ctx, index) {
            ...
            return Dismissible(
              ...
              onDismissed: (_) {
                setState(() {
                  attemptDelete(_classes[index]["key"].toString(), context);
              ...
              child: Card(
                ...
                child: ListTile(
                  leading: CircleAvatar(
                    child: Text(_classes[index]["key"].toString()),
                  ),
                  title: Text(_classes[index]["value"]),
  ...
  getClasses() async {
    final response = await http
        .get(Uri.parse('http://10.0.2.2:8081/api/v1/class/'));
    if (response.statusCode == 200) {
      var jsonResponse = json.decode(response.body);
      print(jsonResponse);
